...
/*   Updated: 2022/05/08 21:18:58 by schung           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	lexer_token_bl(char *str, int *i, t_list **l_token)
{
		...
		|| (str[*i] == '|' && str[*i + 1] == '|'))
	{
		...
		token = token_create(token_str, TOK_BL);
		if (token == NULL)
		{
			free(token_str);
			return (print_error(SHELL_NAME, NULL, NULL, strerror(ENOMEM)));
...
int	lexer_token_bracket(char *str, int *i, t_list **l_token)
{
	...
	if (str[*i] == '(' || str[*i] == ')')
	{
		...
		if (token_str == NULL)
			return (print_error(SHELL_NAME, NULL, NULL, strerror(ENOMEM)));
		if (str[*i] == '(')
			token = token_create(token_str, TOK_B_BRACKET);
		else
			token = token_create(token_str, TOK_E_BRACKET);
		if (token == NULL)
		{
			free(token_str);
			return (print_error(SHELL_NAME, NULL, NULL, strerror(ENOMEM)));
